from utils.utils_distinct_gp_per_unit.data_processing_unit import reconstruct_hyperparameters, reconstruct_hyperparameters_elbo, reconstruct_hyperparameters_elbo_2
...
def save_and_print_gp_hyperparams(iteration, num_iterations, losses, flat_parameters_initial, initial_hyperparameters,
                                  ...
                                  failure_modes_sensors=None, start_time=None):
    # Only execute the following block at the specified print interval
    if (iteration + 1) % print_interval == 0:
        # Reconstruct hyperparameters and lambda_hyp for each (sensor, failure_mode) combination
        optimized_hyperparameters, optimized_lambda_hyp = reconstruct_hyperparameters(
            flat_parameters_initial.detach(), initial_hyperparameters, failure_modes_sensors)
        ...
        # Dynamically add losses for each (sensor, failure_mode) combination
        fm_losses = {}
        for i, (sensor, fm) in enumerate(failure_modes_sensors):
            fm_losses[(sensor, fm)] = losses[-1][i + 1]
            data_to_save[f"{sensor}_{fm}_Loss"] = fm_losses[(sensor, fm)]

        # Include lambda_hyp values for each (sensor, failure_mode) combination in the data to save
        for (sensor, fm), lambda_val in optimized_lambda_hyp.items():
            data_to_save[f"Lambda_hyp_{sensor}_{fm}"] = lambda_val.item()

        # Include the hyperparameters in the data to save
        for key, params in optimized_hyperparameters.items():
            unit, sensor, fm = key
            for param_name, param_value in params.items():
                data_to_save[f"{unit}_{sensor}_{fm}_{param_name}"] = param_value.item()
        ...
        # Print the iteration, elapsed time, remaining time, parameters, and losses
        param_strings = [f"{unit}_{sensor}_{fm}_{param_name}: {param_value.item()}"
                         for (unit, sensor, fm), params in optimized_hyperparameters.items()
                         for param_name, param_value in params.items()]
        lambda_strings = [f"Lambda_hyp_{sensor}_{fm}: {lambda_val.item()}"
                          for (sensor, fm), lambda_val in optimized_lambda_hyp.items()]
        fm_loss_strings = [f"{sensor}_{fm}_Loss: {fm_losses[(sensor, fm)]}" for (sensor, fm) in failure_modes_sensors]
...
def save_and_print_parameters_3(iteration, num_iterations, losses, flat_parameters_initial, metadata, main_folder_path,
                              file_name, save_interval=1000, print_interval=1, header_written=False, start_time=None):
    # Only execute the following block at the specified print interval
    if (iteration + 1) % print_interval == 0:
        total_loss, cox_loss, kl_loss, kl_b_rho_loss = losses[-1]
        ...
        print(
            f"Iteration {iteration + 1}, Elapsed Time: {formatted_elapsed_time}, "
            f"Estimated Time Remaining: {formatted_remaining_time}, Total Loss: {total_loss}, Cox Loss: {cox_loss} KL Loss: {kl_loss},'kl_b_rho loss': {kl_b_rho_loss} , " + ", ".join(param_strings)
        )
