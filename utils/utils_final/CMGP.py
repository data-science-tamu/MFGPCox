...
def get_inducing_points(data, number_of_inducing_points, preferred_device=device):
    ...
    inducing_points = torch.linspace(length.min() - 30 , length.max() + 30, steps=number_of_inducing_points,
...
#                          data, hyperparameters, lambda_hyperparameter_dict, approx_cov, preferred_device=device):
#     inducing_points = get_inducing_points(data.get((sensor, prediction_failure_mode)), number_of_inducing_points)
#     lambda_hyperparameter = lambda_hyperparameter_dict.get((sensor, prediction_failure_mode))
#     k_u_u_matrix = k_u_u(inducing_points, inducing_points.T, lambda_hyperparameter)
#
#     historical_outputs = all_sensor_readings.get((sensor, prediction_failure_mode))
...
# def get_cmgp_predictions(all_sensor_readings, prediction_input, number_of_inducing_points, unit, sensors,
#                          prediction_failure_mode, data, hyperparameters, lambda_hyperparameter_dict, approx_cov,
#                          preferred_device=device):
#     # Initialize dictionaries to store mean and variance predictions for each sensor
#     mean_predictions = {}
#     variance_predictions = {}
#
#     # Loop over each sensor to generate predictions
...
#         k_f_star_f_star_matrix = k_f_f(t_star, t_star.T, alpha, alpha, xi, xi, lambda_hyperparameter)
#
#         k_u_f_stacked = approx_cov[(sensor, prediction_failure_mode)]['k_u_f_stacked']
#         d_plus_noise_matrix = approx_cov[(sensor, prediction_failure_mode)]['d_plus_noise_matrix']
#         a_matrix = approx_cov[(sensor, prediction_failure_mode)]['a']
#
#         mean_prediction = (k_f_star_u_matrix @ torch.linalg.solve(a_matrix, k_u_f_stacked) @
#                            torch.linalg.solve(d_plus_noise_matrix, historical_outputs))
#
#         var_pred = (k_f_star_f_star_matrix - k_f_star_u_matrix @ torch.linalg.solve(k_u_u_matrix, k_u_f_star_matrix) +
#                     k_f_star_u_matrix @ torch.linalg.solve(a_matrix, k_u_f_star_matrix) +
#                     sigma * torch.eye(k_f_star_f_star_matrix.shape[0], device=preferred_device))
#
#         # Store the predictions for this sensor
#         mean_predictions[sensor] = mean_prediction
#         variance_predictions[sensor] = var_pred
#
...
def get_cmgp_predictions(all_sensor_readings, prediction_input, number_of_inducing_points, unit, sensors,
                         prediction_failure_mode, data, hyperparameters, lambda_hyperparameter_dict, approx_cov,
                         preferred_device=device):
    # Initialize tensors to store mean and variance predictions for each sensor
    num_sensors = len(sensors)
    prediction_length = prediction_input.shape[0]

    mean_predictions = torch.zeros((num_sensors, prediction_length, 1), device=preferred_device)
    variance_predictions = torch.zeros((num_sensors, prediction_length, prediction_length), device=preferred_device)
    # Loop over each sensor to generate predictions
    for i, sensor in enumerate(sensors):
        inducing_points = get_inducing_points(data.get((sensor, prediction_failure_mode)), number_of_inducing_points)
        # inducing_points = get_inducing_points(data.get((unit, sensor, prediction_failure_mode)), number_of_inducing_points)
        lambda_hyperparameter = lambda_hyperparameter_dict.get((sensor, prediction_failure_mode))
        k_u_u_matrix = k_u_u(inducing_points, inducing_points.T, lambda_hyperparameter)
        ...
        # Store the predictions in the tensor format
        mean_predictions[i, :, :] = mean_prediction
        variance_predictions[i, :, :] = var_pred

    return mean_predictions, variance_predictions



def get_cmgp_predictions_fm(all_sensor_readings, prediction_input, number_of_inducing_points, unit, sensors,
                         prediction_failure_mode, data, hyperparameters, lambda_hyperparameter_dict, approx_cov,
                         preferred_device=device):
    # Initialize tensors to store mean and variance predictions for each sensor
    num_sensors = len(sensors)
