...
# ths function should be used when consider prior for alpha and beta
    ...
    for failure_mode, data_dict in data_dicts.items():
        ...
        for (unit, sensor, fm) in data_dict.keys():
            ...
            if (sensor, fm) not in initial_hyperparameters:
                # Generate hyperparameters for this unique combination
                initial_hyperparameters[(sensor, fm)] = {
                    'mu_alpha_hat': torch.rand(1, device=preferred_device),
                    'sigma_alpha_hat': torch.rand(1, device=preferred_device),
                    'alpha_theta1_hat': torch.rand(1, device=preferred_device),
                    'beta_theta1_hat': torch.rand(1, device=preferred_device),
                    'alpha_theta2_hat': torch.rand(1, device=preferred_device),
                    'beta_theta2_hat': torch.rand(1, device=preferred_device),
                    'sigma': torch.normal(mean=0, std=0.0001, size=(1,), device=preferred_device)
                }
...
########################################################################################################################
def flatten_hyperparameters_2(hyperparameters, lambda_hyp):
    flat_parameters = []
    ...
    for key, value in hyperparameters.items():
        flat_parameters.extend([
            value['mu_alpha_hat'],
            value['sigma_alpha_hat'],
            value['alpha_theta1_hat'],
            value['beta_theta1_hat'],
            value['alpha_theta2_hat'],
            value['beta_theta2_hat'],
            value['sigma']
        ])
...
def flatten_hyperparameters_3(hyperparameters, lambda_hyp):
    flat_parameters = []
    param_names = []
    ...
    for fm, lambda_val in lambda_hyp.items():
        flat_parameters.append(lambda_val)
        param_names.append(f"lambda_hyp[{fm}]")
    ...
    for key, value in hyperparameters.items():
        ...
        param_names.extend([
            f"{key}_mu_alpha_hat",
            f"{key}_sigma_alpha_hat",
            f"{key}_alpha_theta1_hat",
            f"{key}_beta_theta1_hat",
            f"{key}_alpha_theta2_hat",
            f"{key}_beta_theta2_hat",
            f"{key}_sigma"
        ])

    return torch.cat(flat_parameters), param_names


# Reconstruct hyperparameters from a flat array
def reconstruct_hyperparameters_2(flat_parameters, template, failure_modes):
    lambda_hyp = {}
    hyperparameters = {}
    index = 0
    ...
    for key in template.keys():
        hyperparameters[key] = {
            'mu_alpha_hat': flat_parameters[index].unsqueeze(0),
            'sigma_alpha_hat': torch.exp(flat_parameters[index + 1].unsqueeze(0)),
            'alpha_theta1_hat': torch.exp(flat_parameters[index + 2].unsqueeze(0)),
            'beta_theta1_hat': torch.exp(flat_parameters[index + 3].unsqueeze(0)),
            'alpha_theta2_hat': torch.exp(flat_parameters[index + 4].unsqueeze(0)),
            'beta_theta2_hat': torch.exp(flat_parameters[index + 5].unsqueeze(0)),
            'sigma': flat_parameters[index + 6].unsqueeze(0),
        }
        index += 7
