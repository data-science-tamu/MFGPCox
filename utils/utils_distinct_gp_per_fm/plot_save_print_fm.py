from utils.utils_distinct_gp_per_fm.data_processing_fm import (reconstruct_hyperparameters,
                                                               reconstruct_hyperparameters_elbo,
                                                               reconstruct_hyperparameters_elbo_2,
                                                               reconstruct_hyperparameters_2)
import time
import numpy as np
import os
import torch
...
def save_and_print_gp_hyperparams_2(iteration, num_iterations, losses, flat_parameters_initial, initial_hyperparameters,
                                  main_folder_path, file_name, save_interval=10000,
                                  print_interval=1, header_written=False,
                                  failure_modes=None, start_time=None):
    # Only execute the following block at the specified print interval
    if (iteration + 1) % print_interval == 0:
        # Reconstruct hyperparameters and lambda_hyp for each failure mode
        optimized_hyperparameters, optimized_lambda_hyp = reconstruct_hyperparameters_2(
            flat_parameters_initial.detach(), initial_hyperparameters, failure_modes)

        # Prepare data for saving
        total_loss = losses[-1][0]
        kl_loss = losses[-1][1]
        penalty_loss = losses[-1][2]
        data_to_save = {
            'Iteration': iteration + 1,
            'Total_Loss': total_loss,
        }
        ...
        lambda_strings = [f"Lambda_hyp_{fm}: {lambda_val.item()}" for fm, lambda_val in optimized_lambda_hyp.items()]
        fm_loss_strings = [f"fm{fm}_Loss: {fm_losses[fm]}" for fm in failure_modes]
        print(f"Iteration {iteration + 1}, Total_Loss: {total_loss}, kl_Loss: {kl_loss}, penalty_loss: {penalty_loss}, " +
              ", ".join(fm_loss_strings) +
              f", Elapsed Time: {formatted_elapsed_time}, Estimated Time Remaining: {formatted_remaining_time}, " +
              ", ".join(param_strings + lambda_strings))
...
########################################################################################################################
def save_and_print_gp_hyperparams_3(iteration, num_iterations, losses, flat_parameters_initial, initial_hyperparameters,
                                  main_folder_path, file_name, save_interval=10000,
                                  print_interval=1, header_written=False,
                                  failure_modes=None, start_time=None):
    # Only execute the following block at the specified print interval
