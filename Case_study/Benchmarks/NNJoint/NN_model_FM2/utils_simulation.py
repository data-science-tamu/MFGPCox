...
def zt_1(t, a_b_c_matrix):
    ...
    t = np.atleast_1d(t)
    # Construct t_matrix
    t_matrix = np.c_[np.ones_like(t), t,  t ** 2]
...
def compare_pre_result(tstar_time,unit, model,node_in, smoothed_H0t, min, max):
    ...
    # input test data
    column_test = [2, 3, 4, 5, 6, 7, 8, 9]
    test_data = pd.read_csv('Test_simulation/processed_test_data.csv')
...
#                Ht=[]
                ...
                for m in range(tstar[g],upper):
                    ...
                    col_2 = zt_2(per_t, a_b_c_matrix[:, 1])
                    col_3 = zt_2(per_t, a_b_c_matrix[:, 2])
                    col_4 = zt_2(per_t, a_b_c_matrix[:, 3])
                    col_5 = zt_2(per_t, a_b_c_matrix[:, 4])
                    col_6 = zt_2(per_t, a_b_c_matrix[:, 5])
                    col_7 = zt_2(per_t, a_b_c_matrix[:, 6])
                    col_8 = zt_2(per_t, a_b_c_matrix[:, 7])


                    # Stack columns horizontally -> shape (1000, 2)
                    test_input = np.column_stack((col_1, col_2, col_3, col_4, col_5, col_6, col_7, col_8))
                    test_input = pd.DataFrame(test_input, columns=[f'degradation{i}' for i in range(1, 9)])
                    out_temp = model(torch.tensor(test_input.values).float())
                    out_temp = out_temp.detach().numpy()

                    h0t = smoothed_H0t(per_t)
                    h0t[h0t < 0] = 0
                    h0t[per_t < 154] = 0.00001
