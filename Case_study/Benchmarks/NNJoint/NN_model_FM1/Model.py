...
class Efron_loss_penalty(nn.Module):
    ...
    def forward(self, times, events, risk):
        ...
        for t in range(len(T)):
            ...
            tie_exp_risk = torch.sum(exp_risk[tie_index])
            if tie_counts > 0:
                temp_loss = torch.add(temp_loss.to(device=device), self.penalty * torch.sum(torch.abs(risk[index])))
            ...
            for j in range(tie_counts):
                temp = torch.sub(tie_risk[j],
                                 torch.log(torch.sub(torch.sum(exp_risk[index]), j / tie_counts * tie_exp_risk)))
                temp_loss = torch.sub(temp_loss.to(device=device), temp.to(device=device))

            loss = torch.add(temp_loss.to(device=device), loss.to(device=device))
