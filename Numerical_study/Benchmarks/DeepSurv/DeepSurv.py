...
def extract_last_values(data):
    ...
    columns = ['unit number', 'failure mode', 'duration', 'sensor 1', 'sensor 2', 'event']
...
def prepare_time_varying_data(data):
    ...
    return data[['unit number', 'failure mode', 'duration', 'sensor 1', 'sensor 2', 'event']]
...
def train_model_for_exclusion(exclude_value):
    # Reset random seeds
    np.random.seed(1234)
    torch.manual_seed(123)
    ...
    train_filtered = train_data[train_data.iloc[:, 0] != exclude_value]
    ...
    cols_standardize = ['sensor 1', 'sensor 2']
    cols_leave = ['duration']
    ...
    # Save results
    surv_file_path = f"./surv_sequential_{exclude_value}_{cutoff}.csv"
    ...
    for col in surv.columns[1:]:
        ...
        unit_ids.append(col)
        print(f"Exclude {exclude_value} - Unit {col}: Integral = {integral_value.item()}")
...
for cutoff in [10, 20, 50, 75]:
    ...
    results = {}
    for exclude_value in [2, 1]:
        df_result = train_model_for_exclusion(exclude_value)
        if exclude_value == 2:
            results['FM1_probs'] = df_result
        elif exclude_value == 1:
...
test_data = pd.read_csv(f'./test_data_fm_75.csv')
...
hidden_dim = 32
num_classes = 2  # Failure mode 1 and 2
...
# Training loop
for epoch in range(100):  # Increase epochs for better accuracy
    clf_model.train()
...
for t in time_thresholds:
    ...
    all_results = []  # List to hold results for all units

    for unit in units:
        unit_data = test_data_filtered[test_data_filtered['unit number'] == unit]

        features = unit_data[['sensor 1', 'sensor 2']].values.astype('float32')
        ...
        result = {'ID': unit, 'fm_1_prob': fm_1_prob, 'fm_2_prob': fm_2_prob}
