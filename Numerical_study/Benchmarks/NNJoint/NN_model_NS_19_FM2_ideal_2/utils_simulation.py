...
def zt_1(t, a_b_c_matrix):
    ...
    t = np.atleast_1d(t)
    # Construct t_matrix
    t_matrix = np.c_[np.ones_like(t), t ** 0.8 * np.cos(t), t ** 2 + t]
...
def load_data(Normalize=True):
    ...
    for item in Unit:
        ...
        base_df.loc[ts + ir - 1, 'died'] = int(status[item - 61])
        ts = ts + ir
...
#                Ht=[]
            ...
            # est_mrl[test_unit-61, g] = sum(mrl / samples)
