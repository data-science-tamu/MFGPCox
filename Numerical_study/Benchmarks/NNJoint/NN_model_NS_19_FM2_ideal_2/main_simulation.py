...
is_train = False
use_gpu = True
path = f'trained_model_ideal_loss{loss_type}.pth'
if is_train:
    # train model
    if use_gpu:
        model = model.cuda()
...
if loss_type == 1:
    smoothed_H0t = UnivariateSpline(failure_time, tie_counts / cumulative_sum, s=0.1)

if loss_type == 2:
    smoothed_H0t = UnivariateSpline(failure_time, tie_counts / cumulative_sum, s=0.18)
...
# # est_sz,true_sz=utils_simulation.compare_pre_result(model, node_in, smoothed_H0t, min, max)
#
# est_sz, est_sz_lower, est_sz_upper, mrl, survival_probs = utils_simulation.compare_pre_result(50,
#                                                                                               59,
#                                                                                               model,
#                                                                                               node_in,
#                                                                                               smoothed_H0t,
#                                                                                               min,
#                                                                                               max)
# time_steps = range(50, 50 + len(est_sz[0]))
#
...
for tstar_time in [75]:
    ...
    # for unit in chain(range(51, 61), range(111, 121)):
    for unit in [55,59]:
