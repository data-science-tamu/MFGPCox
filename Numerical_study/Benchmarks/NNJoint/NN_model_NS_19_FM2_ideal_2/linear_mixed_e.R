...
# Read your data
TEMP <- read.csv("Training/processed_data_fm2_ideal.csv", header=TRUE)
...
for (i in names) {
  ...
  if (i == "degradation1") {
    # For degradation1: [1, t^0.8*cos(t), t^2 + t]
    fixed_terms  <- c("I(obstime^0.8 * cos(obstime))", "I(obstime^2 + obstime)")
    random_terms <- c("I(obstime^0.8 * cos(obstime))", "I(obstime^2 + obstime)")
  ...
  # Example: i ~ I(obstime^0.8*cos(obstime)) + I(obstime^2 + obstime)
  fml_fixed <- as.formula(
    paste(i, "~", paste(fixed_terms, collapse = "+"))
  )
  
  # Construct formula for the random effects
  # Example: ~ I(obstime^0.8*cos(obstime)) + I(obstime^2 + obstime) | id
