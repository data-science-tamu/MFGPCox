...
for t in [20, 50, 75]:
    DeepSurv = pd.read_csv(f'../Benchmarks/DeepSurv/save/t_{t}/all_units_probabilities.tsv', sep='\t')
    DeepSurv.columns = ['unit', 'DeepSurv_p1', 'DeepSurv_p2']

    DeepBranched = pd.read_csv(rf"../Benchmarks/DeepBrch/output_t{t}_same_set_paper/classifier_result.csv")
    DeepBranched = DeepBranched.drop(columns=DeepBranched.columns[0])
    DeepBranched.columns = ['DeepBranched_p1', 'DeepBranched_p2']

    NN_joint_ideal = pd.read_csv(f'../Benchmarks/NNJoint/NN_model_NS_19_ideal_prob_2/fm_probabilities/t_{t}/fm_probabilities_t_{t}.csv')
    ...
    NN_joint_mis = pd.read_csv(f'../Benchmarks/NNJoint/NN_model_NS_19_ideal_prob_2/fm_probabilities/t_{t}/fm_probabilities_t_{t}.csv')
    NN_joint_mis = NN_joint_mis.drop(columns=NN_joint_mis.columns[0])
    NN_joint_mis.columns = ['NN_joint_mis_p1', 'NN_joint_mis_p2']

    cmgp = pd.read_csv(f'../Prediction/sensor 1_sensor 2_t_{t}_NNloss2_c-fm_sp-ind_cprob_median_c_elbo/failure_mode_probs_{t}.csv')
