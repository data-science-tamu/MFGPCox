...
plot_dir = 'fm1_data/time_series_plots'
os.makedirs(plot_dir, exist_ok=True)

summary_plot_dir = 'fm1_data/summary_plots'
...
mu_1 = torch.tensor([2.2, 0.1, 0.01], device=device)
mu_2 = torch.tensor([1.7, 0.115, 0.013], device=device)

Sigma_1 = torch.tensor([[0.8 * scaling_factor, 3e-5, 4e-5],
...
time = torch.linspace(0, 200, steps=1000)  # Shape: [T]

# Avoid division by zero by ensuring no zero values in time in case I define basis with t in denominator
time[time == 0] = 1e-8
...
#     # B_i1, B_i2: shape [N]
#     # t_star: scalar
#     # Returns: survival of shape [N, T]
#
#     device = t.device  # Ensure everything is on the same device
#
#     # Create a tensor of linearly spaced points for all times in `t`
#     ls = torch.linspace(t_star, t.max(), num_points).to(device)  # Shape: [num_points]
#
#     # Create a mask to select points up to each `t`
#     t_expanded = t.unsqueeze(0)  # Shape: [1, T]
#     mask = ls.unsqueeze(1) <= t_expanded  # Shape: [num_points, T]
#
#     # Compute the hazard function for all time points (broadcasted over units)
#     vals = true_hazard(ls.unsqueeze(-1), B_i1, B_i2)  # Shape: [num_points, N]
#
#     # Apply the mask to zero out values beyond each `t`
#     vals = vals * mask.unsqueeze(-1)  # Shape: [num_points, T, N]
#
#     # Compute the integral using trapz over the `num_points` axis
#     integral_approx = torch.trapz(vals, ls, dim=0)  # Shape: [T, N]
#
#     # Compute the survival probability
#     survival = torch.exp(-integral_approx.T)  # Shape: [N, T]
#
#     return survival
...
# def compute_f_for_points(B_i1_samples, B_i2_samples, t_star, num_points=1000):
#     t_values = torch.linspace(0.1, 200, num_points, device=device)
#     f_values = f(t_values, B_i1_samples, B_i2_samples, t_star, num_points)
#     return t_values, f_values
...
# B_i1_samples = torch.tensor([[1.2811e+00, 1.2088e-01, 1.2428e-03]], device='cuda:0')
# B_i2_samples = torch.tensor([[1.7496e+00,  9.8939e-02, -1.3583e-04]], device='cuda:0')
# N = B_i1_samples.shape[0]
# t_min = torch.tensor(0., device='cuda:0')
# t_max = torch.tensor(200., device='cuda:0')
# B_i1_samples = B_i1_samples.to(device)
# B_i2_samples = B_i2_samples.to(device)
# t_star = t_min.to(device)
#
...
#
#         f_t_prime = f(t_prime, B_i1_samples[i:i + 1], B_i2_samples[i:i + 1], t_star, 1000).to(device)
...
num_samples = 0
max_attempts = 100000  # Set a maximum number of attempts per unit
...
plt.xlabel('Time')
plt.ylabel('Density')
plt.title('Density Function, Histogram of Samples, and Uniform Density - sample unit from failure 1')
plt.legend()
plt.grid(True)
plt.show()
...
density_folder = 'fm1_data/density_plots'
hazard_folder = 'fm1_data/hazard_plots'
survival_folder = 'fm1_data/survival_plots'
...
for i in range(N):
    ...
    while attempts < max_attempts:
        ...
        # if Y1_i[0] + 15. <= Y1_i[-1] and Y2_i[0] + 20. <= Y2_i[-1]:
        # Store the samples
...
for i in range(N):
    unit_number = i + 1  # unit numbers are indexed from 1
    ...
    # Create DataFrame
    df = pd.DataFrame({
        'failure mode': 1,  # failure mode is 1 for this data
...
csv_filename = os.path.join(output_dir, 'all_units_time_series_fm1.csv')
all_units_df.to_csv(csv_filename, index=False)

# Save B values and failure times
B_values_data = []
for i in range(N):
    unit_number = i + 1
    B_i1_values = B_i1_samples_list[i].cpu().numpy()
...
B_csv_filename = os.path.join(output_dir, 'B_values_failure_times_fm1.csv')
B_values_df.to_csv(B_csv_filename, index=False)

# Create log file to save sigma, lambda, and alpha values
log_filename = os.path.join(output_dir, 'parameters_log_fm1.txt')
