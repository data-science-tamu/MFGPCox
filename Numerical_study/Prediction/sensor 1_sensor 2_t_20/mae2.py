...
all_units = list(chain(range(51, 61), range(111, 121)))
excluded_units_for_later_t = {53, 113}
units = [u for u in all_units if u not in excluded_units_for_later_t] if t in {50, 75} else all_units

for unit_name in units:
    print(f"Processing unit: {unit_name}")

    tsv_file_path = rf"C:\Users\sina.aghaee\Desktop\GitLab\CM\numerical_study\numerical_study_19\sensor 1_sensor 2_t_{t}_NNloss2_c-fm_sp-ind_cprob_median_c_elbo_WI12_12000\sp_{t}_exact_ns10000\{unit_name}.tsv"
...
ranges = {
    "fm1(51,61)": [u for u in units if 51 <= u <= 61],
    "fm2(111,121)": [u for u in units if 111 <= u <= 121],
