...
save_directory = f'{sensors_str}_t_{t}__new_run'
...
for unit in unit_f:
    ...
    #remove following(just to avoid nan for test)
    if unit[0] in range (51,61):
        final_prob1 = torch.tensor(1.0, device=device)
        final_prob2 = torch.tensor(0.0, device=device)

    else:
        final_prob1 = torch.tensor(0.0, device=device)
        final_prob2 = torch.tensor(1.0, device=device)
...
#
# loaded_hyperparameters[(56, 'sensor 2', 1)]['alpha'] = loaded_hyperparameters[(53, 'sensor 2', 1)]['alpha']
# loaded_hyperparameters[(56, 'sensor 2', 1)]['xi'] = loaded_hyperparameters[(53, 'sensor 2', 1)]['xi']
#
