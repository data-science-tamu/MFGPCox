...
for unit_name in chain(range(51, 61), range(111, 121)):
    ...
    try:
        ...
        trunc_index = torch.where(mask == False)[0].min().item() -7 if not mask.all() else len(times)
        ...
        true_mrl_values.append({"Unit": unit_name, "True_MRL": true_mrl})
...
true_mrl_df = pd.DataFrame(true_mrl_values)
true_mrl_df.to_csv(f"true_mrl_values_{t}.csv", index=False)
ranges = {
